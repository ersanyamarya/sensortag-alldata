...
console.log("bhej de bhai");

SensorTag.discover(function(sensorTag) {
  console.log('discovered: ' + sensorTag);
  data.device.id = sensorTag.id;
  data.device.type = sensorTag.type;
  ...
  sensorTag.on('disconnect', function() {
    ...
    client.on('connect', function() {
      client.publish('sanyam/data', payload);
    });
  ...
  async.series([
    ...
    function(callback) {
      if (USE_READ) {
        ...
        sensorTag.readIrTemperature(function(error, objectTemperature,
          ambientTemperature) {
          ...
          data.irTemp.object = objectTemperature.toFixed(1);
          data.irTemp.ambient = ambientTemperature.toFixed(1);
          callback();
        });
      } else {
    ...
    function(callback) {
      if (USE_READ) {
        ...
        sensorTag.readAccelerometer(function(error, x, y, z) {
          ...
          data.accelerometer.x = x.toFixed(1);
          data.accelerometer.y = y.toFixed(1);
          data.accelerometer.z = z.toFixed(1);
          callback();
        });
      } else {
    ...
    function(callback) {
      if (USE_READ) {
        ...
        sensorTag.readHumidity(function(error, temperature, humidity) {
          console.log('\ttemperature = %d °C', temperature.toFixed(
            1));
          console.log('\thumidity = %d %', humidity.toFixed(1));
          data.humid.temperature = temperature.toFixed(1);
          data.humid.humidity = humidity.toFixed(1);
          callback();
        });
      } else {
    ...
    function(callback) {
      if (USE_READ) {
        ...
        sensorTag.readMagnetometer(function(error, x, y, z) {
          ...
          data.magnetometer.x = x.toFixed(1);
          data.magnetometer.y = y.toFixed(1);
          data.magnetometer.z = z.toFixed(1);
          callback();
        });
      } else {
      ...
      if (sensorTag.type === 'cc2540') {
        async.series([
          ...
          function(callback) {
            ...
            sensorTag.readTestConfiguration(function(error,
              configuration) {
